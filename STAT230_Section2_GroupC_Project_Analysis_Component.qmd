...
Since we removed *species* as a variable, we did not create boxplots of *total cup points* vs. quantitative predictors, with species as a factor.
...
No Outliers Condition: There are some outliers in the scatterplots between each quantitative predictor and total_cup_points, so it does not meet the no outliers condition. 
...
```{r}
subset_model <- lm(total_cup_points ~ aftertaste + body + flavor, data = coffee_data)

library(car)
vif(subset_model)

final_model <- lm(total_cup_points ~ flavor + aftertaste + body, data = coffee_data)
summary(final_model)
